...
shaft_speeds = []
shaft_angles = []
target_rate = []  # List to store position data for plotting
x_time = []
...
try:
    ...
    last_rate = None  # Track the last sent position
    while True:
        ...
        if iteration % update_interval == 0:
            ...
            # Generate sine wave position between 100 and 120 degrees
            rate = 100

            # Send position setpoint
            motorVEL = actuator.sendVelocitySetpoint(rate)
            target_rate.append(rate)
            last_rate = rate  # Update the last sent position
        else:
    ...
    with open("motor_data.csv", "w", newline="") as csvfile:
        ...
        # Write the header
        csvwriter.writerow(["Time (s)", "Current (A)", "Shaft Speed (rpm)", "Shaft Angle (deg)", "Target Speed (rpm)"])
        # Find the minimum length of all lists to avoid IndexError
        min_length = min(len(x_time), len(currents), len(shaft_speeds), len(shaft_angles), len(target_rate))
        # Write the data
        for i in range(min_length):
            csvwriter.writerow([x_time[i], currents[i], shaft_speeds[i], shaft_angles[i], target_rate[i]])
